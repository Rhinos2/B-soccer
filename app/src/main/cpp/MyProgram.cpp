...
GLshort MYPROGRAM::PROGRAM_create(AssetReader& assetReader, GLchar *vertex_shader_filename, GLchar *fragment_shader_filename, bool debug_shader) {


    if(0 != assetReader.readAssetFile(vertex_shader_filename)) {
        Log::error("Error reading asset file");
        return -1;
    }
    vertex_shader.SHADER_init(vertex_shader_filename, GL_VERTEX_SHADER );
    if(1 != vertex_shader.SHADER_compile( assetReader.getaBuffer(), debug_shader )){
        __android_log_print(ANDROID_LOG_ERROR, "", "[ %s ]\n%s", name, "Error reading compiling ver shader file.");
        return -1;
    }
    if(0 != assetReader.readAssetFile(fragment_shader_filename)) {
        Log::error("Error reading asset file");
        return -1;
    }

    fragment_shader.SHADER_init(fragment_shader_filename, GL_FRAGMENT_SHADER );
    if(1 != fragment_shader.SHADER_compile( assetReader.getaBuffer(), debug_shader)){
