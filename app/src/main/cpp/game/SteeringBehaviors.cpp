...
//
//------------------------------------------------------------------------
SteeringBehaviors::SteeringBehaviors(PlayerBase*  agent, GameApp* world, SoccerBall*  ball):
                                  
             m_pPlayer(agent),
             m_iFlags(0),
             m_dMultSeparation(6.0f),  //SeparationCoefficient
             m_bTagged(false),
             m_dViewDistance( 0.6f), //SeparationDistance
             m_pBall(ball),
             m_dInterposeDist(0.0f),
             m_Antenna(0.3f,Vector2D())
{
}
...
//------------------------------------------------------------------------
float SteeringBehaviors::ForwardComponent()
{
...
//------------------------------------------------------------------------
float SteeringBehaviors::SideComponent()
{
...
//------------------------------------------------------------------------
Vector2D SteeringBehaviors::Arrive(Vector2D    target, Deceleration deceleration)
{
  ...
  if (dist > 0)
  {
    ...
    const float DecelerationTweaker = 5.0f;//0.4;
...
//------------------------------------------------------------------------
Vector2D SteeringBehaviors::Interpose(const SoccerBall* ball, Vector2D  target, float  DistFromTarget)
{
  Vector2D Target1(ball->OldPos() - target);
  if(Target1.LengthSq() < DistFromTarget*DistFromTarget) {
      DistFromTarget = Target1.Length();
  }
////////////////////////////////////////////////////////////////////////////////////////
    Vector2D Target = target + Vec2DNormalize(ball->Pos() - target) * DistFromTarget;
    Vector2D ToTarget = Target - m_pPlayer->Pos();
    ...
    if (dist > 0) {
        //because Deceleration is enumerated as an int, this value is required
        //to provide fine tweaking of the deceleration..
        const float DecelerationTweaker = 3.3;

        //calculate the speed required to reach the target given the desired
        //deceleration
        float speed = dist / ((float) slow * DecelerationTweaker);
