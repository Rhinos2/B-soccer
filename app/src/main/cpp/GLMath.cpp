...
MYTRANS::MYTRANS(){
    ...
    //bias mat
    biasM = glm::mat4(0.0f);
    biasM[0][0] =biasM[1][1] = biasM[2][2] = 0.5f;
    biasM[3][3] = 1.0f;
    biasM[0][3] = biasM[1][3] = biasM[2][3] = 0.5f;
    //
    MVm.push_back(init);
    MVm.push_back(init); // MV 4 level deep
    MVm.push_back(init);
    MVm.push_back(init);
    PRJm.push_back(init); //PRJ 2 deep
    PRJm.push_back(init);
...
void MYTRANS::SetPerspective(glm::mat4 PrjMat){
    PRJm[PRJ_pointer] = PRJm[PRJ_pointer] * PrjMat;
}
...
void MYTRANS::SetLookAtOld(glm::vec3 eye, glm::vec3 center, glm::vec3 up){
    glm::mat4 Vm =  glm::lookAt(eye, center, up);
    MVm[MV_pointer] = MVm[MV_pointer] * Vm;
}
void MYTRANS::SetLookAt(glm::mat4 Vm){
    //Vm =  glm::lookAt(eye, center, up);
    MVm[MV_pointer] = MVm[MV_pointer] * Vm;

}

void MYTRANS::ChangeLookAt(glm::vec3 eye){
    glm::mat4 translate = glm::translate(-eye);

    MVm[MV_pointer] = MVm[MV_pointer] * translate;
